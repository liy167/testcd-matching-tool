...
try:
    ...
    EXCEL_PATH = r"Z:\projects\utility\metadata\SDTM Terminology.xls"
    MAPPING_FILE = r"Z:\projects\utility\macros\09_metadata\TEST_TESTCD_mapping.xlsx"
    CACHE_DIR = r"Z:\projects\utility\metadata\testcd_embedding"
...
class LabTestMatcher:
    ...
    def __init__(self, excel_path: str = None, mapping_file: str = None, model_name: str = "paraphrase-multilingual-MiniLM-L12-v2", cache_dir: str = None):
        ...
        Args:
            excel_path: Excel文件路径（默认从config.py或环境变量读取）
            mapping_file: 映射文件路径（默认从config.py或环境变量读取）
            model_name: embedding模型名称
            cache_dir: embedding缓存目录（默认从config.py或环境变量读取）
        """
        self.excel_path = excel_path if excel_path is not None else EXCEL_PATH
        self.mapping_file = mapping_file if mapping_file is not None else MAPPING_FILE
        self.model = None
